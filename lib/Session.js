...
function getSession() {
    ...
    const sid = cookie['sid'] || generateID()
    const file_path = this.getConf('session')['PATH_ROOT'] + sid

    return new Promise(res => {
        fs.exists(file_path, exists => {
            if (exists) {
                return fs.readFile(file_path, 'UTF-8', (err, file_data) => {
                    const data = JSON.parse(file_data)
                    const cur_time = +new Date() / 1e3 - (+data.creatTime)
...
class Session {
    ...
    set(data) {
        ...
        return new Promise((res, rej) => {
            this._save(err => err ? rej(err) : res())
        })
    }
    _save(cb) {
